...
2~6인 서버 권한형 카드 심리전 웹게임입니다. V2는 속도·숨김 동률·시선/허점 규칙을 제거하고, `10장 덱 → 5장 시작 손패 → 라운드마다 최대 3장 비공개 예약 → 단계별 동시 판정` 구조를 사용합니다.
...
- 시작/최대 손패 5장, 라운드 시작 시 손패가 몇 장 비었어도 최대 1장만 드로우
- 한 라운드에 0~3장을 선택 순서대로 자동 예약하고 필요할 때만 순서를 편집한 뒤 60초 안에 확정
- 같은 단계의 공격·방어·회피·반격·회복·피해는 동시 기준으로 판정
- 매 3라운드마다 직업 카드 2장 + 공용 카드 1장 중 정확히 2장을 60초 안에 선택
...
테스트는 카드/캐릭터 데이터, 덱 생명주기, 연속 자동 큐와 순서 편집, 단계 동시성, 방어·회피·반격·합 순차 룰렛, 사망 후 예약 취소, 보상/덱 교체, 비공개 뷰, 타이머, 채팅, 재접속, Socket.IO 동기화와 프로덕션 health/CORS를 검증합니다.

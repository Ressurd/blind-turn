...
방 코드, 닉네임, reconnectToken, 비공개 손패와 카드 큐가 응답에 없어야 합니다. Railway healthcheck는 새 배포가 `200`을 반환한 뒤 트래픽을 전환하며, 배포 후의 지속 모니터링을 대신하지는 않습니다.
...
4. 캐릭터 선택, 준비, 게임 시작, 카드 큐 확정, 라운드 판정, 보상과 재경기를 확인합니다.
5. 게임 중 한 브라우저를 새로고침해 자신의 손패·큐·확정 상태·남은 시간이 복원되는지 확인합니다.
